...
`;
const readmeTemplate = (seed, theme, tokens) => `# Prompt-to-UI Export
...
export async function buildExportZip({ spec, html, seed, theme, themeTokens, }) {
    const zip = new JSZip();
    zip.file('page.html', html);
    zip.file('page.json', JSON.stringify(spec, null, 2));
    const component = `import React from "react"\n\n${nextJsTemplate}\n\n${nextJsComponent}`;
    zip.file('nextjs-page.tsx', component);
    zip.file('README.md', readmeTemplate(seed, theme ?? spec.theme, themeTokens));
