...
export function renderThemedHTML(spec: PageSpec): string {
  const esc = (s:string)=>s.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')
  const T = themeClasses((spec as any).theme)
  const accentFromSpec = (spec as any).accent
  const tokens = (spec as any).themeTokens || {}
  const accent = esc(tokens.accent || accentFromSpec || T.accent)
  const radius = esc(tokens.radius || '10px')
  const fontKey = (tokens.font || 'system') as 'system'|'inter'|'serif'
  const fontFamily =
    fontKey === 'inter'
  ...
  const featureItems = spec.features.items.map(it=>{
    if (typeof it === 'string') return `<li class="${T.card} rounded-token"><span>${esc(it)}</span></li>` 
    const icon = (it as any).icon ? `<img src="${esc((it as any).icon)}" alt="" class="inline-block mr-2 h-6 w-6 align-middle">`  : ''
    return `<li class="${T.card} rounded-token">${icon}<span class="align-middle">${esc((it as any).label)}</span></li>` 
  }).join('\n')
  ...
  const faq = spec.faq ? `
    <section id="faq" class="space-y-4">
      ...
      <dl class="space-y-3">
        ${spec.faq.items.map(i=>`<div class="${T.card} rounded-token"><dt class="font-semibold">${esc((i as any).q ?? (i as any).question)}</dt><dd class="text-slate-600 mt-1">${esc((i as any).a ?? (i as any).answer)}</dd></div>` ).join('')}
      </dl>
    </section>` : ''
...
<body class="${T.body} font-token" data-theme="${esc(((spec as any).theme as string) || 'minimal')}">
