...
export async function generateFromPrompt(
  ...
  let styleModeParam = styleModeOrUndefined as any
  let variationParam = variationStrategyOrUndefined as any
  const VARS = new Set(['none', 'reverse-features', 'shuffle-pricing', 'both', 'auto'])
  if (typeof styleModeParam === 'string' && VARS.has(styleModeParam)) {
    variationParam = styleModeParam
    styleModeParam = undefined
  }

  const styleMode = (styleModeParam as 'auto' | 'explicit' | 'seeded' | undefined) ?? (autoStyle ? 'auto' : (explicitTheme || explicitTokens ? 'explicit' : 'none'))
  ...
  let appliedStrategy: 'none' | 'reverse-features' | 'shuffle-pricing' | 'both' | 'auto' = (variationParam as any) || 'auto'
  try {
